...
class BookingController extends AppController
{
	...
	public function paymentProcess(){
		...
		$contact_lname = $_SESSION['contact_lname'] = $this->request->data['contact_lname'];
...
<table cellpadding="0" cellspacing="0" border="0" align="center" style="margin-top:20px;">
	...
	<tr><td colspan="2"><a href="http://labs.myoneh.com/">GO BACK & TRY AGAIN</a></td></tr>
</table>
<?php
...
break;
	...
	//DO NOTHING
            ...
            //if(!empty($this->request->data)){
			  ...
			  //if($test_type=="test_package"){
				...
				$booking_data['contact_lname'] = $contact_lname = $_SESSION['contact_lname'];
				...
				if($this->Booking->save($booking_data)){
                     ...
                     if($this->BookingTest->saveAll($booking_test)){
						...
						$vouncher_code = $clinic_name_code.$month_val.$booking_count; */
						...
						do {
							$vouncher_code = '';
							for ($i = 0; $i < 9; $i++) {
								$vouncher_code .= rand(0, 1) ? rand(0, 9) : chr(rand(ord('A'), ord('Z')));
							}
							//echo $random;
							$sql_find_id = "select count(*) from bookings where voucher_unique_id='$random'";
							//echo $sql_find_id."<br />";
							$autobookingid = $this->Booking->query($sql_find_id);
							$autoidcount =  count($autobookingid);
							//echo $autoidcount;
						}
						while($autoidcount!=1);
 ...
 //echo $pdf_full_path;  exit;                               
				...
				'email_content'=>$email_content,
                        ...
                        $email->emailFormat('html');
                        $email->from($email_from);
                        $email->to('info@onehealth.solutions');
						$email->AddCC('arihant.jain@onehealth.solutions');
						//echo $email; exit;
                        $email->subject($email_subject);
                        $email->viewVars(array(
				'email_content'=>$email_content,
