...
(function($) 
{
	$.fn.vasplus_post_scroller = function(options) {
		...
		var vpb_config = { 
			...
			srchArrItms			: '',
			vpb_no_more         : '<div class="loading_msg"><span><img src="img/favicon.png" width="45" height="45"></span> NO MORE RESULTS.</div>', // This is the message shown to the user when the post is finished
			vpb_load_more       : '<div class="loading_msg"><span><img src="img/loader.GIF" width="45" height="45"></span> FINDING MORE RESULTS FOR YOU, BEAR WITH US.</div>', // This is the message shown to the user when set auto scroll to false to load more data    
			...
			function vasplus_load_more_data_system() {
				if($vpb_config.vpb_url == "") { alert('Error: the page to load data was not passed.\nThank You!'); }
					...
					$.post($vpb_config.vpb_url, {
						...
						vpb_start  : vpb_start,
						srchArrItms   : $vpb_config.srchArrItms
			...
			if( $vpb_config.vpb_auto_load == true ) // Auto Scroll
			{
				$(window).scroll(function() {
					if($(window).scrollTop() + $(window).height() > $this.height() && !vpb_process_started) {
						...
						$this.find('#'+$vpb_config.vpb_loading_div_id).html('<div class="loading_msg"><span><img src="img/loader.GIF" width="45" height="45"></span> FINDING MORE RESULTS FOR YOU, BEAR WITH US</div>');
						setTimeout(function() {
							if(vpb_start < 10){
							//	alert('am here at 79');
							}else{
